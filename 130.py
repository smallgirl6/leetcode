class Solution:
    def solve(self, board: List[List[str]]) -> None:
        ...
        for i in range(column_len): 
         ...
         # 檢查矩陣所有行(row)的第一行和最後一行的所有元素 
        #  ☑         ☑
        #　 X N N N N X
        #   X N N N N X
        #   X N N N N X
        #   X N N N N X             
        for i in [0, column_len-1 ]: # 只會遍歷兩個值：0 和 column_len-1。會查看第一列和最後一列
        ...
        while queue:
            ...
            if 0 <= x < column_len and 0 <= y < row_len and board[x][y] == 'O':  
                board[x][y] = 'E' #能夠連通的 'O'，都會被標記為 'E'
                # 每找到一個 'O'，它周圍可能也是 'O' ，加入到一個待檢查的清單裡
                for dx, dy in [(-1, 0), (1, 0), (0, -1), (0, 1)]: #檢查點的上下左右四個相鄰點
